...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="days365bit/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/days365bit/guestbook.git'
            }
        }
        stage('Build') {
            steps {
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'days365bit.j@gmail.com')
